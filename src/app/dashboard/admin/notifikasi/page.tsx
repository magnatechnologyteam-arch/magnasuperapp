import { redirect } from "next/navigation";
import { BellRing } from "lucide-react";
import { getCurrentProfile } from "@/lib/supabase/server";
import { ToastProvider } from "@/components/ui/ToastProvider";
import { NotificationBroadcastForm } from "@/components/admin/NotificationBroadcastForm";

/**
 * Tahap 32 — halaman "Kirim Notifikasi", HANYA akses penuh (pola sama
 * seperti Status Sistem/Katalog Produk admin). Terpisah dari Kelola
 * Pengguna karena ini bukan soal akun, tapi soal komunikasi ke tim.
 */
export default async function NotifikasiAdminPage() {
  const profile = await getCurrentProfile();
  if (!profile || profile.division !== "all") {
    redirect("/dashboard");
  }

  return (
    <div className="p-4 md:p-8">
      <div className="animate-fade-up flex items-start gap-4">
        <div className="relative shrink-0">
          <span
            className="absolute -inset-1.5 animate-pulse rounded-2xl bg-gradient-to-br from-indigo-500 to-violet-500 opacity-30 blur-lg"
            aria-hidden
          />
          <div className="relative grid h-12 w-12 place-items-center rounded-2xl bg-gradient-to-br from-indigo-500 to-violet-500 text-white shadow-sm">
            <BellRing className="h-6 w-6" />
          </div>
        </div>
        <div>
          <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-indigo-500 dark:text-indigo-400">
            Admin
          </p>
          <h1 className="mt-0.5 text-2xl font-extrabold tracking-tight text-zinc-900 dark:text-white">
            Kirim Notifikasi
          </h1>
          <p className="mt-1.5 text-sm text-zinc-500 dark:text-zinc-400">
            Kirim pengumuman langsung ke perangkat staf yang sudah mengaktifkan notifikasi — terpisah dari notifikasi
            otomatis (booking baru, stok menipis, dst) yang sudah berjalan di tiap modul.
          </p>
        </div>
      </div>

      <div className="mt-6 max-w-lg">
        <ToastProvider>
          <NotificationBroadcastForm />
        </ToastProvider>
      </div>
    </div>
  );
}
